...
public class WebOrderTest {
	...
	/*
	 * Step 1. Navigate to loginpage Step 2. Assert that you are on loginpage Step
	 * 3. Login using valid credentials Step 4. Click on view all products Step 5.
	 * Verify following products are displayed: MyMoney FamilyAlbum ScreenSaver Step
	 * 6. Verify prices and discounts : MyMoney $100 8% FamilyAlbum $80 15%
	 * ScreenSaver $20 10%
	 */
	...
	public void productInformation() throws InterruptedException {
		...
		String month = data.number().numberBetween(1, 9) + "";
		String year = data.number().numberBetween(18, 26) + "";
		informationProductPage.expireDate.sendKeys("0" + month + "/" + year);
		informationProductPage.processButton.click();
		allOrdersPage.viewAllOrders.click();
		assertTrue(allOrdersPage.listOfAllOrders.isDisplayed(), "List Of All Orders label is not displayed");
		assertTrue(informationProductPage.date.getText().equals("07/09/2018"));
